...
public class LuraphDevirtualizer {
    ...
    public LuaChunk process() {
        ...
        // If decodeChunk not found, try alternative identification
        if (decodeChunk == null) {
            identifyFunctionsFallback();
        }
        ...
        for (int i = 0; i < children.size(); i++) {
            ...
            if (node instanceof LocalDeclare) {
                ...
                try {
                    ...
                    System.out.println("WARNING: Could not evaluate expression: " + e.getMessage());
                ...
                try {
                    ...
                    String vtoName = null;
                    Object condObj = stmt.condition;
                    if (condObj instanceof Variable) {
                        vtoName = ((Variable)condObj).symbol();
                    } else {
                        System.out.println("WARNING: ForStep condition is not a Variable, skipping");
                        continue;
                    }
                    
                    LuaValue toVal = vals.get(vtoName);
                    if (toVal == null) {
                        System.out.println("WARNING: Could not find loop variable: " + vtoName);
                        continue;
                    }
                    
                    int init = 1;
                    int to = (int)toVal._nv;
                ...
                if (stmt.block.getChildren(If.class).size() > 0) {
                    ...
                    System.out.println("WARNING: Could not process ForStep while loading chunk: " + e.getMessage());
                }
            }
    ...
    private void getInstructionAndStringDecryptionKeys() {
        if (vmRun == null || decodeChunk == null) {
            System.out.println("WARNING: Could not retrieve encryption keys (vmRun or decodeChunk is null).");
            return;
        }
        ...
        if (idx == -1) {
            System.out.println("WARNING: Could not find string and instruction decryption keys in VM");
            return;
        }
    ...
    private int getDecodeChunkConstantStatementIdx() {
        ...
        // Fallback: if no ForStep has If statements, pick first ForStep
        if (idx == -1) {
            for (int i = 0; i < decodeChunk.block.stmts.size(); i++) {
                Statement stmt = decodeChunk.block.stmts.get(i);
                if (stmt instanceof ForStep) {
    ...
    private void getDecodeChunkConstantIndices() {
        ...
        if (constantStmtIdx < 0 || constantStmtIdx >= decodeChunk.block.stmts.size()) {
            System.out.println("WARNING: Could not get constant indices");
            return;
        }
        ...
        if (stmt == null) {
            // Fallback: if no If statement, try to extract from other statements
            
            // Try to find variable assignments
            List<Node> assigns = constantStep.block.getChildren(Assign.class);
            if (assigns.size() > 0) {
                Assign firstAssign = (Assign)assigns.get(0);
                Variable var = (Variable)firstAssign.first(Variable.class);
                if (var != null && var.suffixes.size() > 0) {
            ...
            if (last != null) {
                Variable lastVar = (Variable)last.first(Variable.class);
                if (lastVar != null && lastVar.suffixes.size() > 0) {
                    constantTableIdx = lastVar.suffixes.get(0).expOrName.line();
                }
            }
        } else {
    ...
    private void getDecodeChunkDebugIndices() {
        for (Node stmt : decodeChunk.block.getChildren(ForStep.class)) {
            ...
            if (children.size() == 1) {
                ...
                if (call != null && call.varOrExp.line().equals(getDwordName)) {
                    Assign assign = (Assign)children.get(0);
                    if (assign != null) {
                        Variable var = (Variable)assign.first(Variable.class);
                        if (var != null && var.suffixes.size() > 0) {
                            debugTableIdx = var.suffixes.get(0).expOrName.line();
                        }
                    }
                }
            }
    ...
    private void getDecodeChunkInstructionIndicies() {
        for (Node stmt : decodeChunk.block.getChildren(ForStep.class)) {
            ...
            if (call != null && call.varOrExp.line().equals(getInstructionName)) {
                List<Node> assign = block.getChildren(Assign.class);

                for (Node s : assign) {
                    FunctionCall gbitsCall = (FunctionCall)s.first(FunctionCall.class);
                    Variable assignVar = (Variable)s.first(Variable.class);
                    if (assignVar != null && assignVar.suffixes.size() > 0) {
                        String idx = assignVar.suffixes.get(0).expOrName.line();
                        ...
                        if (gbitsCall != null && gbitsCall.varOrExp.line().equals(getBitsName)) {
                            for (NameAndArgs nargs : gbitsCall.nameAndArgs) {
                                ...
                                if (list != null && list.exprs.size() >= 3 &&
                                    list.exprs.get(1) instanceof Number && list.exprs.get(2) instanceof Number) {
    ...
    private void getDecodeChunkPrototypeIndicies() {
        for (Node stmt : decodeChunk.block.getChildren(ForStep.class)) {
            ...
            if (children.size() == 1) {
                FunctionCall call = (FunctionCall)children.get(0).first(FunctionCall.class);
                if (call != null && call.varOrExp.line().equals(decodeChunkName)) {
                    Assign assign = (Assign)children.get(0);
                    if (assign != null) {
                        Variable var = (Variable)assign.first(Variable.class);
                        if (var != null && var.suffixes.size() > 0) {
                            prototypeTableIdx = var.suffixes.get(0).expOrName.line();
                        }
                    }
                }
            }
    ...
    private void luraphDecodeString() {
        if (luraphVmString == null) {
            System.out.println("WARNING: Luraph VM string not found in source code. Skipping VM bytecode decoding.");
            return;
        }
    ...
    private void getInstructionOpcodeIndices() throws RuntimeException {
        ...
        if (!instructionToOpcodeMapping.containsKey(VMOp.RETURN)) {
            System.out.println("WARNING: VM does not contain RETURN instruction or instruction mapping is empty.");
            System.out.println("The bytecode could not be devirtualized (possibly over-optimized or corrupted).");
            return;
        }
    ...
    private void identifyFunctionsFallback() {
        // Try to identify functions by their structure when pattern matching fails
        List<Function> allFunctions = new ArrayList<>();
        collectAllFunctions(root, allFunctions);
        
        // Sort by complexity: functions with more ForSteps are likely decode functions
        List<Function> functionsByComplexity = new ArrayList<>(allFunctions);
        functionsByComplexity.sort((f1, f2) -> {
            List<Node> c1 = f1.block != null ? f1.block.getChildren(ForStep.class) : new ArrayList<>();
            List<Node> c2 = f2.block != null ? f2.block.getChildren(ForStep.class) : new ArrayList<>();
            return Integer.compare(c2.size(), c1.size()); // Descending order
        });
        
        // Try to assign functions based on complexity and structure
        for (Function func : functionsByComplexity) {
            List<Node> forSteps = func.block != null ? func.block.getChildren(ForStep.class) : new ArrayList<>();
            int forStepCount = forSteps.size();
            int paramCount = func.params.names.size();
            
            // Identify decodeChunk: has 2-4 ForSteps in main block
            if (decodeChunk == null && forStepCount >= 2 && forStepCount <= 4) {
                decodeChunk = func;
            }
        }
        
        if (decodeChunk == null) {
            System.out.println("WARNING: Could not identify decodeChunk!");
        }
    }
    
    private void collectAllFunctions(Node node, List<Function> functions) {
        ...
        if (children != null) {
            for (Node child : children) {
                collectAllFunctions(child, functions);
            }
        }
    }
    
    private boolean hasComplexBlockStructure(Function func) {
        // Check if function has multiple ForStep loops (typical of decodeChunk)
        int forStepCount = 0;
        for (Node child : func.block.getChildren(ForStep.class)) {
            forStepCount++;
        }
        return forStepCount >= 2;
    }
